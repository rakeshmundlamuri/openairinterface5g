...
struct VRTSIMMultiTestCase {
  ...
  int server_rx;
  int num_ues;
  std::vector<int> client_tx;
  std::vector<int> client_rx;
 ...
 protected:
  ...
  void SetUp() override
  {
    ...
    for (int i = 0; i < param.num_ues; i++) {
      ...
      client_configs[i].tx_num_channels = param.client_tx[i];
      client_configs[i].rx_num_channels = param.client_rx[i];
...
TEST_P(VRTSIMMultiUETest, TransparentChannel)
{
  ...
  for (int u = 0; u < param.num_ues; u++) {
    client_tx_samples[u].resize(param.client_tx[u], std::vector<c16_t>(nsamps));
    client_tx_ptrs[u].resize(param.client_tx[u]);
    for (int a = 0; a < param.client_tx[u]; a++) {
      for (int i = 0; i < nsamps; i++) {
  ...
  for (int u = 0; u < param.num_ues; u++) {
    ASSERT_EQ(client_devices[u].trx_write_func(&client_devices[u], 0, client_tx_ptrs[u].data(), nsamps, param.client_tx[u], 0),
              nsamps);
  }
  ...
  for (int u = 0; u < param.num_ues; u++) {
    std::vector<std::vector<c16_t>> client_rx_samples(param.client_rx[u], std::vector<c16_t>(nsamps));
    std::vector<void *> client_rx_ptrs(param.client_rx[u]);
    for (int a = 0; a < param.client_rx[u]; a++) {
      client_rx_ptrs[a] = client_rx_samples[a].data();
    }

    ASSERT_EQ(client_devices[u].trx_read_func(&client_devices[u], &rx_ts, client_rx_ptrs.data(), nsamps, param.client_rx[u]),
              nsamps);
    EXPECT_EQ(rx_ts, 0);

    // Verify DL mapping for UE u
    int num_dl_mapped = std::min(param.server_tx, param.client_rx[u]);
    for (int a = 0; a < num_dl_mapped; a++) {
      for (int i = 0; i < nsamps; i++) {
...
INSTANTIATE_TEST_SUITE_P(MultiUEVariations,
                         VRTSIMMultiUETest,
                         ::testing::Values(VRTSIMMultiTestCase{2, 2, 2, {2, 1}, {2, 2}, {"2x2", "1x2"}},
                                           VRTSIMMultiTestCase{2, 2, 2, {1, 1}, {2, 2}, {"1x2", "1x2"}}));
